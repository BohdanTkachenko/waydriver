...
- Every session op is bounded by a hard timeout so a wedged session (e.g. a window that never composites, stalling a screenshot's GStreamer pipeline or a D-Bus input call indefinitely) can't hang the MCP client. `--op-timeout-secs` / `WAYDRIVER_OP_TIMEOUT_SECS` (default `30`) is the *server-wide* per-op ceiling applied in `run_action` (covering input, locator actions, queries, value reads) and in the capture tools that bypass `run_action` (`take_screenshot`, `take_element_screenshot`, `compare_element_to_baseline`, via `UiTestServer::bounded`). Every session-scoped tool also accepts a per-call `timeout_ms` override (an `OpTimeout` `#[serde(flatten)]`-ed into each params struct, surfaced as a sibling field on the tool schema; `kill_session` is the lone exception — it keeps its own `KILL_TIMEOUT`). The override resolves through `UiTestServer::op_budget(timeout_ms, extend)`: for element-driven tools (`extend = true`) `timeout_ms` is the locator auto-wait, applied via `tools::locate`, and the backstop is lifted to `timeout_ms + default` so it never preempts the wait; for direct/capture/OCR tools (`extend = false`) `timeout_ms` *is* the hard ceiling. Omitting it uses the server default. Past the budget the in-flight future is dropped and the call returns `Error::Timeout` — the same shape as `kill_session`'s 5s budget. Wait-style tools (`wait_for_stdout_line`, `launch_secondary_instance`) call `run_action_within` with their caller-supplied wait *added* to the op budget, so an intentional long wait is never preempted by the backstop. The op timeout drops the *future* but cannot cancel a native call blocked in C — notably `pipewiresrc` connecting to a wedged session inside `capture::grab_png_sync`, which holds the process-wide `GRAB_PNG_LOCK` (the lock serializes the `PIPEWIRE_REMOTE`/`XDG_RUNTIME_DIR` env mutation the connect reads, so it can't be released while a connect is mid-flight). So the capture paths take that lock via `lock_capture(CAPTURE_LOCK_TIMEOUT)` (20s bounded acquire, not a plain blocking `.lock()`): a screenshot/recording that wedges holds the lock, but every *subsequent* capture fails fast with `Error::Timeout` instead of queueing behind it forever — confining a hard wedge to "captures error until restart" rather than "every future capture, on every session, hangs." True recovery from a hard native wedge would need subprocess isolation (a killable child) and is out of scope.
