...
impl Session {
    ...
    pub fn find_by_role(self: &Arc<Self>, role: Role, name: &str) -> Locator {
        self.locate(&find_by_role_name_xpath(role.element_name(), name))
    }

    /// Locator matching an element by typed [`Role`] and toolkit `id`. For
    /// example, `find_by_role_id(Role::Text, "username")` compiles to
    /// `//Text[@id='username']`. Prefer this over [`find_by_id`](Self::find_by_id)
    /// to disambiguate an id reused across roles, or for locale-stable lookups
    /// where the accessible name varies but the id doesn't.
    pub fn find_by_role_id(self: &Arc<Self>, role: Role, id: &str) -> Locator {
        self.locate(&find_by_role_id_xpath(role.element_name(), id))
    }
}
...
fn find_by_role_id_xpath(role: &str, id: &str) -> String {
    format!("//{}[@id={}]", role, xpath_literal(id))
}
...
mod tests {
    ...
    #[test]
    fn find_by_role_id_xpath_composes_role_and_id() {
        assert_eq!(
            find_by_role_id_xpath("Text", "username"),
            "//Text[@id='username']"
        );
    }

    #[test]
    fn find_by_role_id_xpath_escapes_id_through_xpath_literal() {
        // The id flows through `xpath_literal`, so a quote switches quoting.
        assert_eq!(
            find_by_role_id_xpath("Button", "it's"),
            "//Button[@id=\"it's\"]"
        );
    }
