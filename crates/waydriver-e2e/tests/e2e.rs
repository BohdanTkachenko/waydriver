...
async fn find_by_role_resolves_gtk4_widgets() -> anyhow::Result<()> {
    ...
    // Role::CheckBox is a divergent role: the cache tags a check box `CheckBox`
    // (the reference spelling) but the walk tags it `Checkbox`. The typed helper
    // compiles a union, so it resolves the widget regardless of which snapshot
    // serves the lookup — where a single tag would miss one and need a walk.
    let check = session
    ...
    // box `CheckBox` — the reference spelling the Role::CheckBox union matches.
    let cache_tree = session.dump_tree_cached().await?;
    assert!(
        cache_tree.contains("<CheckBox") && cache_tree.contains("name=\"agree-check\""),
        "premise: the cache tags the check box `CheckBox`, the reference spelling \
         the Role::CheckBox union must match"
    );
