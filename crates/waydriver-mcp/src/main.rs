...
impl UiTestServer {
    ...
    async fn inspect_ui(
        ...
        let tree = atspi_client::dump_app_tree(
    ...
    async fn click_element(
        ...
        // Use AT-SPI action (accurate, no coordinates needed)
    ...
    #[tool(description = "Find a UI element by its accessible name and return its details (bus_name, path, role)")]
...
mod tests {
    ...
    impl InputBackend for MockInput {
        ...
        async fn pointer_motion_relative(&self, _dx: f64, _dy: f64) -> waydriver::error::Result<()> {
            Ok(())
        }
    ...
    async fn insert_test_session(srv: &UiTestServer, id: &str, app_name: &str, display: &str) {
        ...
        .await
        .expect("failed to create test session (needs D-Bus session bus)");
    ...
    async fn take_screenshot_not_found() {
        let s = server();
        let err = s
            .take_screenshot(session_id("bogus"))
            .await
            .unwrap_err();
        assert!(err.message.contains("session not found"));
    }
    ...
    async fn pointer_click_default_button() {
        ...
        let text = content_text(&result);
        // BTN_LEFT = 0x110
        assert!(text.contains("0x110"), "expected BTN_LEFT default, got: {text}");
    }
